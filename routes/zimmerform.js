...
router.post("/createZimmer", async (req, res) => {
    ...
    if (getZimmerartInUnterkunft.rows.length == 0) {
        ...
        const insertZimmerartInUnterkunft = await client.query(
            ...
            "VALUES ($1,$2);",
            [getZimmerartId.rows[0].zimmerartid, getUnterkunftId.rows[0].unterkunftid]
        );
        const insertZimmer = await client.query(
            "INSERT INTO zimmer (zimmername, zimmerartid, unterkunftid,preis,anzahlpersonen) " +
            "VALUES ($1,$2,$3,$4,$5);",
            [req.body.zimmername, getZimmerartId.rows[0].zimmerartid, getUnterkunftId.rows[0].unterkunftid, req.body.preis, req.body.personen]
        );
    }
    else {
        const insertZimmer = await client.query(
            "INSERT INTO zimmer (zimmername, zimmerartid, unterkunftid,preis,anzahlpersonen) " +
            "VALUES ($1,$2,$3,$4,$5);",
            [req.body.zimmername, getZimmerartInUnterkunft.rows[0].zimmerartid, getZimmerartInUnterkunft.rows[0].unterkunftid, req.body.preis, req.body.personen]
        );
    }
        ...
        for (let i = 0; i < verpflegungen.length; i++) {
            try {
                const insertVerpflegungen = await client.query(
                    ...
                    "VALUES ($1,$2,$3);",
                    [verpflegungen[i].einrichtungsid, getZimmerartInUnterkunft.rows[0].zimmerartid, getZimmerartInUnterkunft.rows[0].unterkunftid]
                );
            }
            catch (err) {
        ...
        for (let i = 0; i < ausstattungen.length; i++) {
            try {
                const insertAusstatungen = await client.query(
                    ...
                    "VALUES ($1,$2,$3);",
                    [ausstattungen[i].ausstattungid, getZimmerartInUnterkunft.rows[0].zimmerartid, getZimmerartInUnterkunft.rows[0].unterkunftid]
                );
            }
            catch (err) {
                //nicht schlimm nur key error
...
async function getFormData(type) {
    ...
    switch (type) {
        ...
        case "zimmerausstattung":
            data = await client.query("SELECT ausstattungsname FROM zimmerausstattung");
            for (let i = 0; i < data.rows.length; i++) {
                htmlData += '<div class="input-checkbox equipment-tag"><div class="checkbox-form"><input type="checkbox" class="zimmercheckboxen" id="' + data.rows[i].ausstattungsname + '" />&nbsp;</div><div class="chekbox-form"><p class="styled-p">' + data.rows[i].ausstattungsname + '</p></div></div>';
            }
            break;
        case "verpflegung":
            data = await client.query("SELECT verpflegungname FROM verpflegung");
            for (let i = 0; i < data.rows.length; i++) {
                htmlData += '<div class="input-checkbox equipment-tag"><div class="checkbox-form"><input type="checkbox" class="verpflegungcheckboxen" id="' + data.rows[i].verpflegungname + '" />&nbsp;</div><div class="chekbox-form"><p class="styled-p">' + data.rows[i].verpflegungname + '</p></div></div>';
            }
            break;
    }
    return htmlData; 
